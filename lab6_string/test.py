...
EPSILON = 0.000001
ITEMS__TEST_DB_FILE = "items__test_db.json"
...
def test_set_ID_dictionary():
    ...
    modified_obj = set_ID_dictionary(object, 2)
    assert get_ID_dictionary(modified_obj) == 2


def test_set_name_dictionary():
    one_object = create_object_dictionary(1, "chair", "made of wood", 23, "masa")
    assert get_name_dictionary(one_object) == "chair"
    modified_obj = set_name_dictionary(one_object, "table")
    assert get_name_dictionary(modified_obj) == "table"


def test_set_description_dictionary():
    one_object = create_object_dictionary(1, "chair", "made of wood", 23, "table")
    assert get_description_dictionary(one_object) == "made of wood"
    modified_obj = set_description_dictionary(one_object, "made of steel")
    assert get_description_dictionary(modified_obj) == "made of steel"


def test_set_price_dictionary():
    one_object = create_object_dictionary(1, "chair", "made of wood", 23, "table")
    assert get_price_dictionary(one_object) == 23
    modified_obj = set_price_dictionary(one_object, 12)
    assert get_price_dictionary(modified_obj) == 12
...
def test_set_location_dictionary():
    ...
    modified_obj = set_location_dictionary(one_object, "roof")
    assert get_location_dictionary(modified_obj) == "roof"


def test_add_object_dictionary():
    one_object = create_object_dictionary(1, "chair", "made of wood", 23, "table")
    list_of_objects = []
    add_object_dictionary(list_of_objects, one_object, ITEMS__TEST_DB_FILE)
...
def test_remove_object_dictionary():
    one_object = create_object_dictionary(1, "chair", "made of wood", 23, "table")
    list = [one_object]
    remove_object_dictionary(list, get_ID_dictionary(one_object), ITEMS__TEST_DB_FILE)
    assert len(list) == 0
...
def test_update_object_dictionary():
    one_object = create_object_dictionary(1, "chair", "made of wood", 23, "table")
    list = [one_object]
    update_object_dictionary(1, list, "table", "made of glass", 34, "room", ITEMS__TEST_DB_FILE)
...
def test_move_object_dictionary():
    ...
    move_object_dictionary(list, "table", "room", ITEMS__TEST_DB_FILE)
...
def test_add_string_dictionary():
    ...
    object_two = create_object_dictionary(2, "table", "made of steel", 42, "room")
    list = [object_one, object_two]
    add_string_dictionary(list, " blue", 24, ITEMS__TEST_DB_FILE)
